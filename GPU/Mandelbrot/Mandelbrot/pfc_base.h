//       $Id: pfc_base.h 37919 2018-10-18 11:08:41Z p20068 $
...
template <typename T> auto & read (std::istream & in, T & obj) noexcept {
   if (in.good ()) {
      in.read (reinterpret_cast <typename std::remove_reference_t <decltype (in)>::char_type *> (&obj), sizeof (T));
   }
...
template <typename T> auto & write (std::ostream & out, T const & obj) noexcept {
   if (out.good ()) {
      out.write (reinterpret_cast <typename std::remove_reference_t <decltype (out)>::char_type *> (const_cast <T *> (&obj)), sizeof (T));
   }
...
template <typename T> auto & read (std::istream & in, gsl::span <T> & span) noexcept {
   if (in.good ()) {
      in.read (reinterpret_cast <typename std::remove_reference_t <decltype (in)>::char_type *> (std::data (span)), span.size_bytes ());
   }
...
template <typename T> auto & write (std::ostream & out, gsl::span <T> const & span) noexcept {
   if (out.good ()) {
      out.write (reinterpret_cast <typename std::remove_reference_t <decltype (out)>::char_type *> (std::data (span)), span.size_bytes ());
   }
